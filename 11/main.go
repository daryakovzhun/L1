...
func intersection2(arr1, arr2 []int) []int {
	...
	set := make(map[int]bool)              //  создаем словарь, который будет выступать в роли множества
	...
	for _, val2 := range big { //  итерируемся по множеству большей длины
		if _, ex := set[val2]; ex { //  проверяем наличие элемента из множества в словаре
			set[val2] = true //  если такое значение есть, то по данному ключу меняем значение на true
		}
	}

	var res []int           //  создаем результирующее множество
	for k, v := range set { //  итерируемся по словарю
		if v {
			res = append(res, k) //  добавляем в результирующее множкство все ключи из словаря, где значение true
		}
	}
	return res //  возвращаем результирующее множество
}
